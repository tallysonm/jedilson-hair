...
function NewAppointmentTab({onComplete}:{onComplete:()=>void}){
  ...
  const {toast}=useToast();
  const qc=useQueryClient();
  const [name,setName]=useState("");const [phone,setPhone]=useState("");const [serviceId,setServiceId]=useState("");const [barberId,setBarberId]=useState("all");const [date,setDate]=useState("");const [time,setTime]=useState("");const [isRecurring,setIsRecurring]=useState(false);const [weekday,setWeekday]=useState("4");const [period,setPeriod]=useState<"this_month"|"next_2_months">("this_month");const [result,setResult]=useState<AdminRecurringResult|null>(null);
  ...
  const resetForm=()=>{setName("");setPhone("");setServiceId("");setBarberId("all");setDate("");setTime("");setIsRecurring(false);setWeekday("4");setPeriod("this_month");setResult(null);};
  ...
  const F=({label,children}:{label:string;children:React.ReactNode})=><div className="space-y-1.5"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{label}</label>{children}</div>;
  ...
  return(
    ...
    <div className="glass-card rounded-3xl p-7 space-y-5">
      <div className="grid grid-cols-2 gap-4"><F label="Cliente"><Input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Nome completo" className="bg-white/5 border-white/8 h-11 rounded-xl text-white" data-testid="input-new-client-name"/></F><F label="Telefone"><Input value={phone} onChange={(e)=>setPhone(e.target.value)} placeholder="(11) 99999-9999" className="bg-white/5 border-white/8 h-11 rounded-xl text-white" data-testid="input-new-client-phone"/></F></div>
      <F label="Serviço"><Select value={serviceId} onValueChange={(v)=>{setServiceId(v);setTime("");}}><SelectTrigger className="bg-white/5 border-white/8 h-11 rounded-xl" data-testid="select-new-service"><SelectValue placeholder="Selecione o serviço"/></SelectTrigger><SelectContent className="bg-[#111] border-white/8">{services.map((s)=><SelectItem key={s.id} value={s.id}>{s.name} — {s.priceLabel}</SelectItem>)}</SelectContent></Select></F>
      <F label="Barbeiro"><Select value={barberId} onValueChange={setBarberId}><SelectTrigger className="bg-white/5 border-white/8 h-11 rounded-xl" data-testid="select-new-barber"><SelectValue placeholder="Qualquer barbeiro"/></SelectTrigger><SelectContent className="bg-[#111] border-white/8"><SelectItem value="all">Qualquer barbeiro</SelectItem>{barbers.filter((b)=>b.active).map((b)=><SelectItem key={b.id} value={String(b.id)}>{b.name}</SelectItem>)}</SelectContent></Select></F>
      <F label="Agendamento recorrente?"><div className="flex gap-2">{[{value:false,label:"Não"},{value:true,label:"Sim — semanal fixo"}].map((opt)=><button key={String(opt.value)} type="button" onClick={()=>{setIsRecurring(opt.value);setDate("");setTime("");}} className={`flex-1 h-10 rounded-xl text-sm font-semibold border transition-all ${isRecurring===opt.value?"bg-accent/15 border-accent text-white":"bg-white/4 border-white/8 text-muted-foreground hover:bg-white/7"}`} data-testid={`admin-toggle-recurring-${opt.value}`}>{opt.label}</button>)}</div></F>
      <AnimatePresence mode="wait">
        {!isRecurring?(<motion.div key="single" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} className="overflow-hidden"><div className="grid grid-cols-2 gap-4"><F label="Data"><Input type="date" value={date} min={new Date().toISOString().split("T")[0]} max={getMaxDate()} onChange={(e)=>handleDateChange(e.target.value)} className="bg-white/5 border-white/8 h-11 rounded-xl text-white" data-testid="input-new-date"/></F><F label="Horário"><Select value={time} onValueChange={setTime} disabled={!date||!serviceId||slotsLoading}><SelectTrigger className="bg-white/5 border-white/8 h-11 rounded-xl" data-testid="input-new-time"><SelectValue placeholder={!date||!serviceId?"Escolha data e serviço":slotsLoading?"Carregando...":"Selecione"}/></SelectTrigger><SelectContent className="bg-[#111] border-white/8 max-h-56 overflow-y-auto">{(slotsData?.slots??[]).map((slot:string)=><SelectItem key={slot} value={slot}>{slot}</SelectItem>)}{!slotsLoading&&date&&serviceId&&(slotsData?.slots??[]).length===0&&<div className="px-3 py-4 text-sm text-muted-foreground text-center">Sem horários</div>}</SelectContent></Select></F></div></motion.div>):
        (<motion.div key="recurring" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} className="overflow-hidden space-y-4">
          ...
          <F label="Dia da semana"><div className="flex gap-2 flex-wrap">{ADMIN_WEEKDAYS.map((d)=><button key={d.value} type="button" onClick={()=>{setWeekday(d.value);setTime("");}} className={`h-9 px-3 rounded-xl text-sm font-semibold border transition-all ${weekday===d.value?"bg-accent border-accent text-white":"bg-white/5 border-white/8 text-muted-foreground hover:text-white"}`} data-testid="admin-select-weekday">{d.short}</button>)}</div></F>
          <F label="Horário fixo"><Select value={time} onValueChange={setTime}><SelectTrigger className="bg-white/5 border-white/8 h-11 rounded-xl" data-testid="admin-select-recurring-time"><SelectValue placeholder="Selecione o horário"/></SelectTrigger><SelectContent className="bg-[#111] border-white/8 max-h-56 overflow-y-auto">{(ADMIN_WEEKDAY_SLOTS[weekday]??ADMIN_WEEKDAY_SLOTS["2"]).map((slot)=><SelectItem key={slot} value={slot}>{slot}</SelectItem>)}</SelectContent></Select></F>
          <F label="Período"><div className="flex gap-2">{ADMIN_PERIOD_OPTIONS.map((opt)=><button key={opt.value} type="button" onClick={()=>setPeriod(opt.value as typeof period)} className={`flex-1 h-10 rounded-xl text-sm font-semibold border transition-all ${period===opt.value?"bg-gold/15 border-gold text-gold":"bg-white/4 border-white/8 text-muted-foreground hover:bg-white/7"}`} data-testid={`admin-period-${opt.value}`}>{opt.label}</button>)}</div></F>
        </motion.div>)}
      </AnimatePresence>
